...
//选号容器用Adapter（双色球的红球）
public class PoolK3PAdapter extends BaseAdapter {
    ...
    private int textColorBgResId;// 选中的文字颜色资源id
    private StringBuffer stringBuffer;

    public PoolK3PAdapter(Context context, int endNum, List<Integer> slectedNums, int slectedBgResId, int textColorBgResId, int type) {
        super();
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = View.inflate(context, R.layout.ball_k3_p_list_bg, null);
        ...
        stringBuffer.setLength(0);//清空StringBuffer
        if (type == 3) {
            stringBuffer.append(position + 1).append(position + 1).append(position + 1);
        } else if (type == 2) {
            stringBuffer.append(position + 1).append(position + 1);
        } else if (type == 1) {
            stringBuffer.append(position + 1);
        }
        holder.ball.setText(stringBuffer.toString());
        ...
        if (slectedNums.contains(position + 1)) {
            ...
            holder.ball.setBackgroundResource(R.mipmap.k3_h_gb_itme);
        }
        return convertView;
    }
